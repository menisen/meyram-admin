          ...
          <div class="modal-body p-24">
            <form @submit.prevent>
              <div class="row">
                ...
                <div class="col-12 mb-20">
                  <label class="form-label fw-semibold text-primary-light text-sm mb-8">Платформа</label>
                  <input v-model="formData.platform" type="text" class="form-control radius-8" placeholder="Введите платформу">
                </div>
                <div class="col-12 mb-20">
                  <label class="form-label fw-semibold text-primary-light text-sm mb-8">Ссылка</label>
                  <input v-model="formData.link" type="text" class="form-control radius-8" placeholder="Введите ссылку">
                </div>
                <div class="col-12 mb-20">
                  <label class="form-label fw-semibold text-primary-light text-sm mb-8">Дата начало</label>
                  <input v-model="formData.start_time" type="date" class="form-control radius-8">
                </div>
...
const saveModal = async () => {
  await api.post('/curator/broadcast/live', formData.value)
    .then((res) => {
...
const editModal = async () => {
  await api.put(`/curator/broadcast/live/${props.modalData?.id}`, formData.value)
    .then((res) => {
      toast.success('Успешно!')
      closeModal(true)
    })

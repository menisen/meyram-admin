            ...
            >
              ...
              <tr role="row">
                <th
                  ...
                  colspan="1"
                  :class="{
                    'dt-ordering-asc': sort.orderBy === 'code' && sort.order === 'asc',
                    'dt-ordering-desc': sort.orderBy === 'code' && sort.order === 'desc'
                  }"
                  aria-sort="ascending"
                  aria-label="Code"
                  tabindex="0"
...
import ModalBlock from './livesModal.vue'
...
const getData = async () => {
  await api.get('/curator/broadcast/live')
    .then((res) => {
...
const removeChild = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await api.delete(`/curator/broadcast/live/${id}`)
      await getData()
